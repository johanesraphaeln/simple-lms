...
def show_nim_not_submit(dict_mhs, dict_submission, dict_activity):
    ...
    for i in dict_activity:
        if dict_activity[i]['Type'] == 'assignment':
            assignment.append(i)

    for i in assignment:
        if i not in dict_submission:
            for j in nim:
                not_submit.append(j)
        else:
            for j in nim:
                if j not in dict_submission[i]:
                    not_submit.append(j)
    
    not_submit = list(dict.fromkeys(not_submit))
    not_submit = sorted(not_submit)

    if not_submit:
        print("Daftar NIM Mahasiswa yang belum submit assignment :")
        ...
        for i in not_submit:
            ...
            print('  ', '-' * 55)
            for j in assignment:
                if j not in dict_submission:
                    activity = dict_activity[j]
                    print('  ', j, end='\t| ')
                    print(activity['Title'], end='\t| ')
                    print(activity['Description'], end='\n')
                elif i not in dict_submission[j]:
                    activity = dict_activity[j]
                    print('  ', j, end='\t| ')
                    print(activity['Title'], end='\t| ')
                    print(activity['Description'], end='\n')
        ...
        print('Semua mahasiswa sudah mensubmit semua assignmentnya!')
        print()
...
show_nim_not_submit(dict_mhs, dict_submission, dict_activity)